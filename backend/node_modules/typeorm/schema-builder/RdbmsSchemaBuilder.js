...
class RdbmsSchemaBuilder {
    ...
    constructor(dataSource) {
        ...
        /**
         * All tables loaded from the database for the current sync operation.
         */
        this.tables = [];
        /**
         * All views loaded from the database for the current sync operation.
         */
        this.views = [];
    }
    // -------------------------------------------------------------------------
    ...
    async build() {
        ...
        const isUsingTransactions = !(this.dataSource.driver.options.type === "cockroachdb") &&
    ...
    get entityToSyncMetadatas() {
        return this.dataSource.entityMetadatas.filter((metadata) => metadata.synchronize &&
    ...
    getTablePath(target) {
        ...
        return this.dataSource.driver.buildTableName(parsed.tableName, parsed.schema ?? this.currentSchema, parsed.database ?? this.currentDatabase);
    }
    /**
    ...
    async dropOldForeignKeys() {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (tableForeignKeysToDrop.length === 0)
                continue;
            this.dataSource.logger.logSchemaBuild(`dropping old foreign keys of ${table.name}: ${tableForeignKeysToDrop
    ...
    async renameColumns() {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            this.dataSource.logger.logSchemaBuild(`renaming column "${renamedTableColumns[0].name}" in "${table.name}" to "${renamedColumn.name}"`);
            await this.queryRunner.renameColumn(table, renamedTableColumns[0], renamedColumn);
        }
    }
    shouldDropIndices(indexMetadata, tableIndex) {
        if (indexMetadata) {
            ...
            if (indexMetadata.isSpatial !== tableIndex.isSpatial)
                return true;
            if (!!this.dataSource.driver.supportedIndexTypes &&
                typeof this.dataSource.driver.compareTableIndexTypes ===
                    "function" &&
                !this.dataSource.driver.compareTableIndexTypes(indexMetadata, tableIndex))
                return true;
            if (!this.dataSource.driver.supportedIndexTypes &&
                indexMetadata.type)
                throw new error_1.TypeORMError(`Current database driver does not support index 'type' property`);
            if (this.dataSource.driver.isFullTextColumnTypeSupported() &&
    ...
    async dropOldIndices() {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            const dropQueries = table.indices
                ...
                this.dataSource.logger.logSchemaBuild(`dropping an index: "${tableIndex.name}" from table ${table.name}`);
        ...
        if (this.dataSource.options.type === "postgres") {
            ...
            for (const metadata of this.viewEntityToSyncMetadatas) {
                const view = this.views.find((view) => this.getTablePath(view) === this.getTablePath(metadata));
                if (!view)
                    continue;
                const dropQueries = view.indices
                    ...
                    .map(async (tableIndex) => {
                    this.dataSource.logger.logSchemaBuild(`dropping an index: "${tableIndex.name}" from view ${view.name}`);
    ...
    async dropOldChecks() {
        ...
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (oldChecks.length === 0)
                continue;
            this.dataSource.logger.logSchemaBuild(`dropping old check constraint: ${oldChecks
    ...
    async dropCompositeUniqueConstraints() {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (compositeUniques.length === 0)
                continue;
            this.dataSource.logger.logSchemaBuild(`dropping old unique constraint: ${compositeUniques
    ...
    async dropOldExclusions() {
        ...
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (oldExclusions.length === 0)
                continue;
            this.dataSource.logger.logSchemaBuild(`dropping old exclusion constraint: ${oldExclusions
    ...
    async createNewTables() {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            const existTable = this.tables.find((table) => this.getTablePath(table) === this.getTablePath(metadata));
            if (existTable)
                continue;
            this.dataSource.logger.logSchemaBuild(`creating a new table: ${this.getTablePath(metadata)}`);
            // create a new table and sync it in the database
            const table = Table_1.Table.create(metadata, this.dataSource.driver);
    ...
    async createViews() {
        for (const metadata of this.viewEntityToSyncMetadatas) {
            ...
            const existView = this.views.find((view) => {
            ...
            if (existView)
                continue;
            this.dataSource.logger.logSchemaBuild(`creating a new view: ${this.getTablePath(metadata)}`);
            // create a new view and sync it in the database
            const view = View_1.View.create(metadata, this.dataSource.driver);
    ...
    async dropOldViews() {
        ...
        for (const view of this.views) {
            ...
            if (viewExpression === metadataExpression)
                continue;
            this.dataSource.logger.logSchemaBuild(`dropping an old view: ${view.name}`);
        ...
        this.views = this.views.filter((view) => !droppedViewsWithDependencies.has(view));
    }
    /**
    ...
    async dropRemovedColumns() {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (droppedTableColumns.length === 0)
                continue;
            this.dataSource.logger.logSchemaBuild(`columns dropped in ${table.name}: ` +
    ...
    async addNewColumns() {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (newTableColumns.length === 0)
                continue;
            this.dataSource.logger.logSchemaBuild(`new columns added: ` +
                newColumnMetadatas
        ...
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (primaryTableColumns.length !== primaryMetadataColumns.length &&
                ...
                const changedPrimaryColumns = primaryMetadataColumns.map((primaryMetadataColumn) => {
                    return new TableColumn_1.TableColumn(TableUtils_1.TableUtils.createTableColumnOptions(primaryMetadataColumn, this.dataSource.driver));
                });
    ...
    async updateExistColumns() {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (!table)
                continue;
            const changedColumns = this.dataSource.driver.findChangedColumns(table.columns, metadata.columns);
            ...
            const newAndOldTableColumns = changedColumns.map((changedColumn) => {
                ...
                const newTableColumnOptions = TableUtils_1.TableUtils.createTableColumnOptions(changedColumn, this.dataSource.driver);
            ...
            if (newAndOldTableColumns.length === 0)
                continue;
            this.dataSource.logger.logSchemaBuild(`columns changed in "${table.name}". updating: ` +
                changedColumns
                    .map((column) => column.databaseName)
                    .join(", "));
    ...
    async createNewIndices() {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (newIndices.length === 0)
                continue;
            if (newIndices.find((idx) => !!idx.type &&
                !this.dataSource.driver.supportedIndexTypes)) {
                throw new error_1.TypeORMError(`Current database driver does not support index 'type' property`);
            }
            this.dataSource.logger.logSchemaBuild(`adding new indices ${newIndices
    ...
    async createNewChecks() {
        ...
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (newChecks.length === 0)
                continue;
            this.dataSource.logger.logSchemaBuild(`adding new check constraints: ${newChecks
                .map((index) => `"${index.name}"`)
                .join(", ")} in table "${table.name}"`);
    ...
    async createCompositeUniqueConstraints() {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (compositeUniques.length === 0)
                continue;
            this.dataSource.logger.logSchemaBuild(`adding new unique constraints: ${compositeUniques
                .map((unique) => `"${unique.name}"`)
                .join(", ")} in table "${table.name}"`);
    ...
    async createNewExclusions() {
        ...
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (newExclusions.length === 0)
                continue;
            this.dataSource.logger.logSchemaBuild(`adding new exclusion constraints: ${newExclusions
                .map((exclusion) => `"${exclusion.name}"`)
                .join(", ")} in table "${table.name}"`);
    ...
    async createForeignKeys() {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (newKeys.length === 0)
                continue;
            const dbForeignKeys = newKeys.map((foreignKeyMetadata) => TableForeignKey_1.TableForeignKey.create(foreignKeyMetadata, this.dataSource.driver));
            this.dataSource.logger.logSchemaBuild(`creating a foreign keys: ${newKeys
                .map((key) => key.name)
    ...
    async dropColumnReferencedForeignKeys(tablePath, columnName) {
        const table = this.tables.find((table) => this.getTablePath(table) === tablePath);
        if (!table)
            return;
        const tablesWithFK = [];
        ...
        for (const loadedTable of this.tables) {
        ...
        if (tablesWithFK.length > 0) {
            for (const tableWithFK of tablesWithFK) {
                this.dataSource.logger.logSchemaBuild(`dropping related foreign keys of ${tableWithFK.name}: ${tableWithFK.foreignKeys
    ...
    async dropColumnCompositeIndices(tablePath, columnName) {
        ...
        if (relatedIndices.length === 0)
            return;
        this.dataSource.logger.logSchemaBuild(`dropping related indices of "${tablePath}"."${columnName}": ${relatedIndices
            .map((index) => index.name)
            .join(", ")}`);
    ...
    async dropColumnCompositeUniques(tablePath, columnName) {
        ...
        if (relatedUniques.length === 0)
            return;
        this.dataSource.logger.logSchemaBuild(`dropping related unique constraints of "${tablePath}"."${columnName}": ${relatedUniques
            .map((unique) => unique.name)
            .join(", ")}`);
    ...
    metadataColumnsToTableColumnOptions(columns) {
        return columns.map((columnMetadata) => TableUtils_1.TableUtils.createTableColumnOptions(columnMetadata, this.dataSource.driver));
    }
    /**
    ...
    async createTypeormMetadataTable(queryRunner) {
        ...
        const typeormMetadataTable = this.dataSource.driver.buildTableName(this.dataSource.metadataTableName, schema, database);
