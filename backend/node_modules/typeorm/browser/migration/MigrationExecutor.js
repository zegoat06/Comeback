...
class MigrationExecutor {
    ...
    async showMigrations() {
        ...
        for (const migration of allMigrations) {
            ...
            if (executedMigration) {
                this.dataSource.logger.logSchemaBuild(`[X] ${executedMigration.id} ${migration.name}`);
            }
            else {
    ...
    async executePendingMigrations() {
        ...
        this.dataSource.logger.logSchemaBuild(`${executedMigrations.length} migrations are already loaded in the database.`);
        this.dataSource.logger.logSchemaBuild(`${allMigrations.length} migrations were found in the source code.`);
        if (lastTimeExecutedMigration)
            this.dataSource.logger.logSchemaBuild(`${lastTimeExecutedMigration.name} is the last executed migration. It was executed on ${new Date(lastTimeExecutedMigration.timestamp).toString()}.`);
        this.dataSource.logger.logSchemaBuild(`${pendingMigrations.length} migrations are new migrations must be executed.`);
        if (this.transaction === "all") {
            ...
            if (migrationsOverridingTransactionMode.length > 0) {
                ...
                this.dataSource.logger.logMigration(`Migrations failed, error: ${error.message}`);
                throw error;
            }
        }
        ...
        try {
            for (const migration of pendingMigrations) {
                ...
                await migration
                    ...
                    this.dataSource.logger.logMigration(`Migration "${migration.name}" failed, error: ${error?.message}`);
                    throw error;
                })
                    .then(async () => {
                    ...
                    this.dataSource.logger.logSchemaBuild(`Migration ${migration.name} has been ${this.fake ? "(fake) " : ""}executed successfully.`);
                });
            }
            // commit transaction if we started it
    ...
    async undoLastMigration() {
        ...
        if (!lastTimeExecutedMigration) {
            this.dataSource.logger.logSchemaBuild(`No migrations were found in the database. Nothing to revert!`);
            // if query runner was created by us then release it
            if (!this.queryRunner)
                await queryRunner.release();
            return;
        ...
        try {
            ...
            this.dataSource.logger.logSchemaBuild(`Migration ${migrationToRevert.name} has been ${this.fake ? "(fake) " : ""}reverted successfully.`);
            // commit transaction if we started it
            if (transactionStartedByUs)
                await queryRunner.commitTransaction();
        }
    ...
    async loadExecutedMigrations(queryRunner) {
        if (this.dataSource.driver.options.type === "mongodb") {
            ...
            const migrationsRaw = await this.dataSource.manager
    ...
    async insertExecutedMigration(queryRunner, migration) {
        ...
        if (this.dataSource.driver.options.type === "mssql") {
            values["timestamp"] = new MssqlParameter_1.MssqlParameter(migration.timestamp, this.dataSource.driver.normalizeType({
                type: this.dataSource.driver.mappedDataTypes
                    .migrationTimestamp,
            }));
            values["name"] = new MssqlParameter_1.MssqlParameter(migration.name, this.dataSource.driver.normalizeType({
    ...
    async deleteExecutedMigration(queryRunner, migration) {
        const conditions = {};
        if (this.dataSource.driver.options.type === "mssql") {
            conditions["timestamp"] = new MssqlParameter_1.MssqlParameter(migration.timestamp, this.dataSource.driver.normalizeType({
                type: this.dataSource.driver.mappedDataTypes
                    .migrationTimestamp,
            }));
            conditions["name"] = new MssqlParameter_1.MssqlParameter(migration.name, this.dataSource.driver.normalizeType({
                type: this.dataSource.driver.mappedDataTypes.migrationName,
            }));
        }
        else {
