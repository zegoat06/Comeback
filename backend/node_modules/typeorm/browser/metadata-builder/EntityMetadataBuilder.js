...
class EntityMetadataBuilder {
    ...
    // Constructor
    // -------------------------------------------------------------------------
    constructor(dataSource, metadataArgsStorage) {
        this.dataSource = dataSource;
        this.metadataArgsStorage = metadataArgsStorage;
        this.junctionEntityMetadataBuilder = new JunctionEntityMetadataBuilder_1.JunctionEntityMetadataBuilder(dataSource);
        this.closureJunctionEntityMetadataBuilder =
            new ClosureJunctionEntityMetadataBuilder_1.ClosureJunctionEntityMetadataBuilder(dataSource);
        this.relationJoinColumnBuilder = new RelationJoinColumnBuilder_1.RelationJoinColumnBuilder(dataSource);
    }
    // -------------------------------------------------------------------------
    // Public Methods
    // -------------------------------------------------------------------------
    ...
    build(entityClasses) {
            ...
            entityMetadata.relations
                ...
                if (uniqueConstraint) {
                    if (DriverUtils_1.DriverUtils.isMySQLFamily(this.dataSource.driver) ||
                        ...
                        if (this.dataSource.driver.options.type ===
                            "mssql") {
                            index.where = index.columns
                                .map((column) => {
                                return `${this.dataSource.driver.escape(column.databaseName)} IS NOT NULL`;
                            })
                                .join(" AND ");
                        }
                        if (this.dataSource.driver.options.type ===
        ...
        entityMetadatas.forEach((entityMetadata) => {
            entityMetadata.indices.forEach((index) => index.build(this.dataSource.namingStrategy));
        });
        // build all unique constraints (need to do it after relations and their join columns are built)
        entityMetadatas.forEach((entityMetadata) => {
            entityMetadata.uniques.forEach((unique) => unique.build(this.dataSource.namingStrategy));
        });
        // build all check constraints
        entityMetadatas.forEach((entityMetadata) => {
            entityMetadata.checks.forEach((check) => check.build(this.dataSource.namingStrategy));
        });
        // build all exclusion constraints
        entityMetadatas.forEach((entityMetadata) => {
            entityMetadata.exclusions.forEach((exclusion) => exclusion.build(this.dataSource.namingStrategy));
        });
        ...
        entityMetadatas
            ...
            entityMetadata.relations
                ...
                .forEach((relation) => {
                this.dataSource.relationLoader.enableLazyLoad(relation, entityMetadata.target.prototype);
            });
        });
        entityMetadatas.forEach((entityMetadata) => {
    ...
    createEntityMetadata(tableArgs) {
        ...
        if (tableInheritance?.pattern === "STI" ||
    ...
    computeEntityMetadataStep1(allEntityMetadatas, entityMetadata) {
        ...
        if (entityInheritance?.column) {
            const discriminatorColumnName = entityInheritance.column?.name ?? "type";
    ...
    computeEntityMetadataStep2(entityMetadata) {
        entityMetadata.embeddeds.forEach((embedded) => embedded.build(this.dataSource));
        entityMetadata.embeddeds.forEach((embedded) => {
            embedded.columnsFromTree.forEach((column) => column.build(this.dataSource));
            embedded.relationsFromTree.forEach((relation) => relation.build());
        });
        entityMetadata.ownColumns.forEach((column) => column.build(this.dataSource));
        ...
        entityMetadata.foreignKeys.forEach((foreignKey) => foreignKey.build(this.dataSource.namingStrategy));
