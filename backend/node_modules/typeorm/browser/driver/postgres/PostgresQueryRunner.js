...
class PostgresQueryRunner extends BaseQueryRunner_1.BaseQueryRunner {
    ...
    async createTable(table, ifNotExists = false, createForeignKeys = true, createIndices = true) {
        ...
        if (table.comment) {
            upQueries.push(new Query_1.Query(`COMMENT ON TABLE ${this.escapePath(table)}` +
    ...
    async changeColumn(tableOrName, oldTableColumnOrName, newColumn) {
        ...
        if (oldColumn.type !== newColumn.type ||
            ...
            if ((newColumn.type === "enum" ||
                ...
                const { extraItems, missingItems } = OrmUtils_1.OrmUtils.getArraysDiff(newColumn.enum, oldColumn.enum);
                const version = this.driver.version;
                // when the only change is new enum value(s) we can use ADD VALUE syntax
                const useAddValueForUp = VersionUtils_1.VersionUtils.isGreaterOrEqual(version, "12.0") &&
                ...
                if (useAddValueForUp) {
                    ...
                    for (const item of extraItems) {
                        ...
                        upQueries.push(new Query_1.Query(`ALTER TYPE ${oldEnumName} ADD VALUE '${escapedValue}'`));
                    }
    ...
    async dropColumn(tableOrName, columnOrName, ifExists) {
        ...
        upQueries.push(new Query_1.Query(`ALTER TABLE ${this.escapePath(table)} DROP COLUMN ${ifExistsClause}"${column.name}"`));
    ...
    async dropUniqueConstraint(tableOrName, uniqueOrName, ifExists) {
        ...
        const up = this.dropUniqueConstraintSql(table, uniqueConstraint, ifExists);
    ...
    async dropUniqueConstraints(tableOrName, uniqueConstraints, ifExists) {
        for (const uniqueConstraint of [...uniqueConstraints]) {
            await this.dropUniqueConstraint(tableOrName, uniqueConstraint, ifExists);
        }
    }
    /**
    ...
    async createExclusionConstraint(tableOrName, exclusionConstraint) {
        ...
        exclusionConstraint.name ??=
            this.dataSource.namingStrategy.exclusionConstraintName(table, exclusionConstraint.expression);
    ...
    async dropExclusionConstraint(tableOrName, exclusionOrName, ifExists) {
        ...
        const up = this.dropExclusionConstraintSql(table, exclusionConstraint, ifExists);
    ...
    async dropExclusionConstraints(tableOrName, exclusionConstraints, ifExists) {
        const promises = exclusionConstraints.map((exclusionConstraint) => this.dropExclusionConstraint(tableOrName, exclusionConstraint, ifExists));
        await Promise.all(promises);
    }
    /**
    ...
    async clearDatabase() {
        ...
        if (this.driver.options.schema) {
            ...
            const [{ currentSchema }] = await this.query(`SELECT current_schema() AS "currentSchema"`);
        ...
        try {
            // drop views
            const views = await this.query(`SELECT quote_ident(schemaname) || '.' || quote_ident(viewname) as "name" ` +
                `FROM "pg_views" WHERE "schemaname" = ANY($1) AND "viewname" NOT IN ('geography_columns', 'geometry_columns', 'raster_columns', 'raster_overviews')`, [schemas]);
            if (views.length > 0) {
                await this.query(`DROP VIEW IF EXISTS ${views.map(({ name }) => name).join(", ")} CASCADE`);
            }
            ...
            if (DriverUtils_1.DriverUtils.isReleaseVersionOrGreater(this.driver, "9.3")) {
                const matViews = await this.query(`SELECT quote_ident(schemaname) || '.' || quote_ident(matviewname) as "name" ` +
                    `FROM "pg_matviews" WHERE "schemaname" = ANY($1)`, [schemas]);
                if (matViews.length > 0) {
                    await this.query(`DROP MATERIALIZED VIEW IF EXISTS ${matViews.map(({ name }) => name).join(", ")} CASCADE`);
                }
            }
            // ignore spatial_ref_sys; it's a special table supporting PostGIS
            // TODO generalize this as this.driver.ignoreTables
            // drop tables
            const tables = await this.query(`SELECT quote_ident(schemaname) || '.' || quote_ident(tablename) as "name" ` +
                `FROM "pg_tables" WHERE "schemaname" = ANY($1) AND "tablename" NOT IN ('spatial_ref_sys')`, [schemas]);
            if (tables.length > 0) {
                await this.query(`DROP TABLE IF EXISTS ${tables.map(({ name }) => name).join(", ")} CASCADE`);
            }
            // drop enum types
    ...
    async loadTables(tableNames) {
        ...
        if (!tableNames) {
            ...
            const tablesCondition = tableNames
                ...
                .join(" OR ");
            const tablesSql = `SELECT "table_schema", "table_name", obj_description((quote_ident("table_schema") || '.' || quote_ident("table_name"))::regclass, 'pg_class') AS table_comment FROM "information_schema"."tables" WHERE ` +
                tablesCondition;
            dbTables.push(...(await this.query(tablesSql)));
        }
        ...
        const columnsCondition = dbTables
            ...
            .join(" OR ");
        const columnsSql = `SELECT columns.*, pg_catalog.col_description((quote_ident(table_catalog) || '.' || quote_ident(table_schema) || '.' || quote_ident(table_name))::regclass::oid, ordinal_position) AS description, ` +
        ...
        return Promise.all(dbTables.map(async (dbTable) => {
            ...
            table.columns = await Promise.all(dbColumns
                ...
                if (dbColumn.is_identity === "YES") {
                    ...
                    const defaultWithoutQuotes = dbColumn["column_default"].replaceAll('"', "");
                    if (defaultWithoutQuotes ===
                        ...
                        tableColumn.default = dbColumn["column_default"].replaceAll(/::[\w\s.[\]\-"]+/g, "");
                        tableColumn.default =
    ...
    createTableSql(table, createForeignKeys) {
        ...
        if (table.exclusions.length > 0) {
            const exclusionsSql = table.exclusions
                ...
                const exclusionName = exclusion.name ??
                    this.dataSource.namingStrategy.exclusionConstraintName(table, exclusion.expression);
        ...
        table.columns
            ...
            .forEach((it) => {
            sql += `; COMMENT ON COLUMN ${this.escapePath(table)}."${it.name}" IS ${this.escapeComment(it.comment)}`;
        });
     ...
     * @param ifExists
     */
    dropViewSql(view, ifExists) {
        const materializedClause = view.materialized ? "MATERIALIZED " : "";
        const ifExistsClause = ifExists ? "IF EXISTS " : "";
        return new Query_1.Query(`DROP ${materializedClause}VIEW ${ifExistsClause}${this.escapePath(view)}`);
    }
    /**
    ...
    async dropEnumTypes(schemaNames) {
        const enums = await this.query(`SELECT quote_ident(n.nspname) || '.' || quote_ident(t.typname) as "name" FROM "pg_type" "t" ` +
            `INNER JOIN "pg_enum" "e" ON "e"."enumtypid" = "t"."oid" ` +
            `INNER JOIN "pg_namespace" "n" ON "n"."oid" = "t"."typnamespace" ` +
            `WHERE "n"."nspname" = ANY($1) GROUP BY "n"."nspname", "t"."typname"`, [schemaNames]);
        if (enums.length > 0) {
            await this.query(`DROP TYPE IF EXISTS ${enums.map(({ name }) => name).join(", ")} CASCADE`);
        }
    }
    /**
    ...
    createIndexSql(table, index) {
        ...
        const columns = index.columnNames
            .map((columnName) => `"${columnName}"`)
            .join(", ");
        return new Query_1.Query(`CREATE ${index.isUnique ? "UNIQUE " : ""}INDEX${index.isConcurrent ? " CONCURRENTLY" : ""} "${index.name}" ON ${this.escapePath(table)} ${indexTypeClause ?? ""} (${columns}) ${index.where ? "WHERE " + index.where : ""}`);
    }
    /**
    ...
    createViewIndexSql(view, index) {
        ...
        const columns = index.columnNames
            .map((columnName) => `"${columnName}"`)
            .join(", ");
        return new Query_1.Query(`CREATE ${index.isUnique ? "UNIQUE " : ""}INDEX "${index.name}" ON ${this.escapePath(view)} ${indexTypeClause ?? ""} (${columns}) ${index.where ? "WHERE " + index.where : ""}`);
    }
    /**
     ...
     * @param ifExists
     */
    dropIndexSql(table, indexOrName, ifExists) {
        ...
        const { schema } = this.driver.parseTableName(table);
        return schema
            ? new Query_1.Query(`DROP INDEX ${concurrent ? "CONCURRENTLY " : ""}${ifExistsClause}"${schema}"."${indexName}"`)
            : new Query_1.Query(`DROP INDEX ${concurrent ? "CONCURRENTLY " : ""}${ifExistsClause}"${indexName}"`);
    }
    /**
     ...
     * @param ifExists
     */
    dropPrimaryKeySql(table, ifExists) {
        ...
        const primaryKeyName = constraintName ??
            this.dataSource.namingStrategy.primaryKeyName(table, columnNames);
        const ifExistsClause = ifExists ? "IF EXISTS " : "";
        return new Query_1.Query(`ALTER TABLE ${this.escapePath(table)} DROP CONSTRAINT ${ifExistsClause}"${primaryKeyName}"`);
    }
    /**
     ...
     * @param ifExists
     */
    dropUniqueConstraintSql(table, uniqueOrName, ifExists) {
        const uniqueName = InstanceChecker_1.InstanceChecker.isTableUnique(uniqueOrName)
            ? uniqueOrName.name
            : uniqueOrName;
        const ifExistsClause = ifExists ? "IF EXISTS " : "";
        return new Query_1.Query(`ALTER TABLE ${this.escapePath(table)} DROP CONSTRAINT ${ifExistsClause}"${uniqueName}"`);
    }
    /**
     ...
     * @param ifExists
     */
    dropCheckConstraintSql(table, checkOrName, ifExists) {
        const checkName = InstanceChecker_1.InstanceChecker.isTableCheck(checkOrName)
            ? checkOrName.name
            : checkOrName;
        const ifExistsClause = ifExists ? "IF EXISTS " : "";
        return new Query_1.Query(`ALTER TABLE ${this.escapePath(table)} DROP CONSTRAINT ${ifExistsClause}"${checkName}"`);
    }
    /**
    ...
    createExclusionConstraintSql(table, exclusionConstraint) {
        let sql = `ALTER TABLE ${this.escapePath(table)} ADD CONSTRAINT "${exclusionConstraint.name}" EXCLUDE ${exclusionConstraint.expression}`;
     ...
     * @param ifExists
     */
    dropExclusionConstraintSql(table, exclusionOrName, ifExists) {
        const exclusionName = InstanceChecker_1.InstanceChecker.isTableExclusion(exclusionOrName)
            ? exclusionOrName.name
            : exclusionOrName;
        const ifExistsClause = ifExists ? "IF EXISTS " : "";
        return new Query_1.Query(`ALTER TABLE ${this.escapePath(table)} DROP CONSTRAINT ${ifExistsClause}"${exclusionName}"`);
    }
    /**
     ...
     * @param ifExists
     */
    dropForeignKeySql(table, foreignKeyOrName, ifExists) {
        const foreignKeyName = InstanceChecker_1.InstanceChecker.isTableForeignKey(foreignKeyOrName)
            ? foreignKeyOrName.name
            : foreignKeyOrName;
        const ifExistsClause = ifExists ? "IF EXISTS " : "";
        return new Query_1.Query(`ALTER TABLE ${this.escapePath(table)} DROP CONSTRAINT ${ifExistsClause}"${foreignKeyName}"`);
    }
    /**
    ...
    buildSequenceName(table, columnOrName) {
        ...
        if (seqName.length > this.dataSource.driver.maxAliasLength) {
            ...
            seqName = `${tableName.slice(0, 29)}_${columnName.slice(0, Math.max(29, 63 - table.name.length - 5))}_seq`;
        }
        return seqName;
    }
    ...
    buildEnumName(table, column, withSchema = true, disableEscape, toOld) {
        ...
        let enumName = column.enumName ?? `${tableName}_${column.name.toLowerCase()}_enum`;
    ...
    async changeTableComment(tableOrName, newComment) {
        ...
        upQueries.push(new Query_1.Query(`COMMENT ON TABLE ${this.escapePath(newTable)} IS ${escapedNewComment}`));
        downQueries.push(new Query_1.Query(`COMMENT ON TABLE ${this.escapePath(table)} IS ${escapedComment}`));
        await this.executeQueries(upQueries, downQueries);
