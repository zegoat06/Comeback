...
class AbstractSqliteDriver {
    ...
    constructor(dataSource) {
         ...
         */
        this.supportedIsolationLevels = AbstractSqliteDriver.supportedIsolationLevels;
        /**
    ...
    compareJsonDefaults(columnMetadata, tableColumn) {
        ...
        if (typeof processedDefault === "string") {
            ...
            if (processedDefault.startsWith("(") &&
                processedDefault.endsWith(")")) {
                processedDefault = processedDefault.slice(1, -1);
            }
            const fnWrapped = processedDefault.match(/^(jsonb|json)\s*\(\s*'((?:''|[^'])*)'\s*\)\s*$/i);
            if (fnWrapped) {
                processedDefault = fnWrapped[2];
            }
            else if (processedDefault.startsWith("'") &&
                processedDefault.endsWith("'")) {
                processedDefault = processedDefault.slice(1, -1);
            }
        }
        if (typeof processedDefault === "string") {
            try {
                const tableDefaultObj = JSON.parse(processedDefault.replaceAll("''", "'"));
                return OrmUtils_1.OrmUtils.deepCompare(columnMetadata.default, tableDefaultObj);
            }
            catch (err) {
            ...
            return OrmUtils_1.OrmUtils.deepCompare(columnMetadata.default, processedDefault);
        }
        return false;
    }
    /**
