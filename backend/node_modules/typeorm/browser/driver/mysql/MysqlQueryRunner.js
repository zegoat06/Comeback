...
class MysqlQueryRunner extends BaseQueryRunner_1.BaseQueryRunner {
    ...
    async dropView(target, ifExists) {
        ...
        if (ifExists) {
            upQueries.push(new Query_1.Query(`DROP VIEW IF EXISTS ${this.escapePath(view)}`));
        }
        else {
    ...
    async renameTable(oldTableOrName, newTableName) {
        ...
        newTable.indices.forEach((index) => {
            const oldIndexName = this.dataSource.namingStrategy.indexName(oldTable, index.columnNames);
    ...
    async changeTableComment(tableOrName, newComment) {
        ...
        upQueries.push(new Query_1.Query(`ALTER TABLE ${this.escapePath(newTable)} COMMENT ${escapedNewComment}`));
        downQueries.push(new Query_1.Query(`ALTER TABLE ${this.escapePath(table)} COMMENT ${escapedComment}`));
        await this.executeQueries(upQueries, downQueries);
        ...
        if ((newColumn.isGenerated !== oldColumn.isGenerated &&
            ...
            if (newColumn.name !== oldColumn.name) {
                ...
                clonedTable.findColumnIndices(oldColumn).forEach((index) => {
                    const oldUniqueName = this.dataSource.namingStrategy.indexName(clonedTable, index.columnNames);
                    // Skip renaming if Index has user defined constraint name
    ...
    async loadTables(tableNames) {
        ...
        const kcuSubquerySql = dbTables
            .map(({ TABLE_SCHEMA, TABLE_NAME }) => {
            return (`SELECT * FROM \`INFORMATION_SCHEMA\`.\`KEY_COLUMN_USAGE\` \`kcu\` ` +
                `WHERE ` +
            ...
            table.columns = await Promise.all(dbColumns
                ...
                if (dbColumn["EXTRA"].indexOf("on update") !== -1) {
                    ...
                    tableColumn.onUpdate = dbColumn["EXTRA"]
                        .slice(dbColumn["EXTRA"].indexOf("on update") +
                        10)
                        .toUpperCase();
                }
