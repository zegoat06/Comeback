...
class MongoDriver {
    ...
    constructor(dataSource) {
         ...
         */
        this.supportedIsolationLevels = MongoDriver.supportedIsolationLevels;
        /**
    ...
    async connect() {
        ...
        this.queryRunner = new MongoQueryRunner_1.MongoQueryRunner(this.dataSource, client);
     ...
     */
    createSchemaBuilder() {
        return new MongoSchemaBuilder_1.MongoSchemaBuilder(this.dataSource);
    }
    /**
    ...
    buildConnectionUrl(options) {
        ...
        if (options.replicaSet) {
            connectionString = `${schemaUrlPart}://${credentialsUrlPart}${options.hostReplicaSet ??
                `${options.host ?? "127.0.0.1"}${portUrlPart}`}/${options.database ?? ""}`;
        }
        else {
