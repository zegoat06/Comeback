...
class SqlServerQueryRunner extends BaseQueryRunner_1.BaseQueryRunner {
        ...
        if ((newColumn.isGenerated !== oldColumn.isGenerated &&
            ...
            if (newColumn.name !== oldColumn.name) {
                ...
                if (oldColumn.default !== null &&
                    ...
                    const oldDefaultName = this.dataSource.namingStrategy.defaultConstraintName(table, oldColumn.name);
                    const newDefaultName = this.dataSource.namingStrategy.defaultConstraintName(table, newColumn.name);
            ...
            if (this.isEnumChanged(oldColumn, newColumn)) {
                ...
                const oldCheck = new TableCheck_1.TableCheck({
                    name: this.dataSource.namingStrategy.checkConstraintName(table, oldExpression, true),
                    expression: oldExpression,
                });
                const newExpression = this.getEnumExpression(newColumn);
                const newCheck = new TableCheck_1.TableCheck({
                    name: this.dataSource.namingStrategy.checkConstraintName(table, newExpression, true),
            ...
            if (newColumn.default !== oldColumn.default) {
                ...
                if (oldColumn.default !== null &&
                    oldColumn.default !== undefined) {
                    const defaultName = this.dataSource.namingStrategy.defaultConstraintName(table, oldColumn.name);
                ...
                if (newColumn.default !== null &&
                    ...
                    const defaultName = this.dataSource.namingStrategy.defaultConstraintName(table, newColumn.name);
    ...
    async createForeignKey(tableOrName, foreignKey) {
        ...
        const metadata = this.dataSource.hasMetadata(table.name)
            ...
            : undefined;
        if (metadata?.treeParentRelation &&
     ...
     */
    async clearTable(tablePath, options) {
        if (options?.cascade) {
            throw new error_1.TypeORMError(`SqlServer does not support clearing table with cascade option`);
        }
            ...
            if (allTablesResults.length > 0) {
                ...
                const foreignKeysSql = Object.entries(tablesByCatalog)
                    ...
                    const conditions = tables
                        ...
                        .join(" OR ");
                    return (`SELECT DISTINCT '${TABLE_CATALOG}' AS "TABLE_CATALOG", ` +
    ...
    async loadTables(tableNames) {
        ...
        if (!tableNames) {
            ...
            const tablesSql = dbDatabases
                .map(({ name }) => {
                return (`SELECT DISTINCT "TABLE_CATALOG", "TABLE_SCHEMA", "TABLE_NAME" ` +
    ...
    removeParenthesisFromDefault(defaultValue) {
        ...
        if (!defaultValue.endsWith(")") || end <= 0)
            return defaultValue;
    ...
    buildCreateColumnSql(table, column, skipIdentity, createDefault, skipEnum) {
        ...
        if (!skipEnum && column.enum) {
            ...
            const checkName = this.dataSource.namingStrategy.checkConstraintName(table, expression, true);
