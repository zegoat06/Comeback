...
class CockroachQueryRunner extends BaseQueryRunner_1.BaseQueryRunner {
        ...
        try {
            ...
            if (err.code === "40001" &&
                ...
                for (const q of this.queries) {
                    this.driver.dataSource.logger.logQuery(`Retrying transaction for query "${q.query}"`, q.parameters, this);
                    result = await this.query(q.query, q.parameters, q.useStructuredResult);
                }
                this.transactionRetries = 0;
                this.storeQueries = true;
                return result;
    ...
    async createDatabase(database, ifNotExists) {
        const up = `CREATE DATABASE ${ifNotExists ? "IF NOT EXISTS " : ""} ${this.driver.escape(database)}`;
    ...
    async dropView(target, ifExists) {
        ...
        await this.executeQueries([
            ...
            ifExists
                ? new Query_1.Query(`DROP VIEW IF EXISTS ${this.escapePath(view)}`)
        ...
        if (columnIndex) {
            ...
            if (columnIndex.isUnique) {
                const unique = new TableUnique_1.TableUnique({
                    name: this.dataSource.namingStrategy.uniqueConstraintName(table, columnIndex.columnNames),
    ...
    async dropColumn(tableOrName, columnOrName, ifExists) {
        ...
        upQueries.push(new Query_1.Query(`ALTER TABLE ${this.escapePath(table)} DROP COLUMN ${ifExists ? "IF EXISTS " : ""}"${column.name}"`));
    ...
    async clearDatabase() {
        ...
        try {
            ...
            const views = await this.query(`SELECT quote_ident(schemaname) || '.' || quote_ident(viewname) as "name" ` +
                `FROM "pg_views" WHERE "schemaname"::STRING = ANY($1::STRING[])`, [schemas]);
            if (views.length > 0) {
                await this.query(`DROP VIEW IF EXISTS ${views.map(({ name }) => name).join(", ")} CASCADE`);
            }
            // drop tables
            const tables = await this.query(`SELECT quote_ident(table_schema) || '.' || quote_ident(table_name) as "name" ` +
                `FROM "information_schema"."tables" WHERE "table_schema"::STRING = ANY($1::STRING[]) AND "table_type" = 'BASE TABLE'`, [schemas]);
            if (tables.length > 0) {
                await this.query(`DROP TABLE IF EXISTS ${tables.map(({ name }) => name).join(", ")} CASCADE`);
            }
            // drop sequences
            const sequences = await this.query(`SELECT quote_ident(sequence_schema) || '.' || quote_ident(sequence_name) as "name" ` +
                `FROM "information_schema"."sequences" WHERE "sequence_schema"::STRING = ANY($1::STRING[])`, [schemas]);
            if (sequences.length > 0) {
                await this.query(`DROP SEQUENCE IF EXISTS ${sequences.map(({ name }) => name).join(", ")}`);
            }
    ...
    async loadTables(tableNames) {
        ...
        const tableSchemas = dbTables
            ...
            .join(", ");
        const enumsSql = `SELECT "t"."typname" AS "name", string_agg("e"."enumlabel", '|' ORDER BY "e"."enumsortorder") AS "value" ` +
            `FROM "pg_enum" "e" ` +
        ...
        return Promise.all(dbTables.map(async (dbTable) => {
            ...
            table.columns = await Promise.all(dbColumns
                ...
                if (dbColumn["crdb_sql_type"].indexOf("COLLATE") !==
                    -1) {
                    tableColumn.collation = dbColumn["crdb_sql_type"].slice(dbColumn["crdb_sql_type"].indexOf("COLLATE") +
                        "COLLATE".length +
                        1);
                    tableColumn.type = tableColumn.type.slice(0, dbColumn["crdb_sql_type"].indexOf("COLLATE") - 1);
                }
                if (tableColumn.type.indexOf("(") !== -1)
                    tableColumn.type = tableColumn.type.slice(0, tableColumn.type.indexOf("("));
    ...
    createTableSql(table, createForeignKeys) {
        ...
        table.indices
            ...
            .forEach((index) => {
            table.uniques.push(new TableUnique_1.TableUnique({
                name: this.dataSource.namingStrategy.uniqueConstraintName(table, index.columnNames),
                columnNames: index.columnNames,
            }));
        });
    ...
    async dropEnumTypes(schemaNames) {
        const enums = await this.query(`SELECT quote_ident(n.nspname) || '.' || quote_ident(t.typname) as "name" FROM "pg_type" "t" ` +
            ...
            `INNER JOIN "pg_namespace" "n" ON "n"."oid" = "t"."typnamespace" ` +
            `WHERE "n"."nspname"::STRING = ANY($1::STRING[]) GROUP BY "n"."nspname", "t"."typname"`, [schemaNames]);
        if (enums.length > 0) {
            await this.query(`DROP TYPE IF EXISTS ${enums.map(({ name }) => name).join(", ")}`);
        }
    }
    /**
    ...
    dropIndexSql(table, indexOrName, ifExists) {
        const indexName = InstanceChecker_1.InstanceChecker.isTableIndex(indexOrName) ||
            ...
            ? indexOrName.name
            : indexOrName;
        return new Query_1.Query(`DROP INDEX ${ifExists ? "IF EXISTS " : ""}${this.escapePath(table)}@"${indexName}" CASCADE`);
    }
    /**
    ...
    dropPrimaryKeySql(table, ifExists) {
        ...
        const primaryKeyName = constraintName ??
            this.dataSource.namingStrategy.primaryKeyName(table, columnNames);
        return new Query_1.Query(`ALTER TABLE ${this.escapePath(table)} DROP CONSTRAINT ${ifExists ? "IF EXISTS " : ""}"${primaryKeyName}"`);
    }
    /**
    ...
    dropCheckConstraintSql(table, checkOrName, ifExists) {
        const checkName = InstanceChecker_1.InstanceChecker.isTableCheck(checkOrName)
            ? checkOrName.name
            : checkOrName;
        return new Query_1.Query(`ALTER TABLE ${this.escapePath(table)} DROP CONSTRAINT ${ifExists ? "IF EXISTS " : ""}"${checkName}"`);
    }
    /**
    ...
    dropForeignKeySql(table, foreignKeyOrName, ifExists) {
        const foreignKeyName = InstanceChecker_1.InstanceChecker.isTableForeignKey(foreignKeyOrName)
            ...
            : foreignKeyOrName;
        return new Query_1.Query(`ALTER TABLE ${this.escapePath(table)} DROP CONSTRAINT ${ifExists ? "IF EXISTS " : ""}"${foreignKeyName}"`);
    }
    /**
