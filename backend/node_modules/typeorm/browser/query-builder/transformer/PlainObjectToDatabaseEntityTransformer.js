...
class PlainObjectToDatabaseEntityTransformer {
    ...
    async transform(plainObject, metadata) {
        ...
        const isMongoDb = this.manager.dataSource.driver.options.type === "mongodb";
        await Promise.all(loadMap.groupByTargetIds().map(async (targetWithIds) => {
            let entities;
            if (isMongoDb) {
                const mongoManager = this
                    .manager;
                entities = await mongoManager.findByIds(targetWithIds.target, targetWithIds.ids);
            }
            else {
                entities = await this.manager
                    .getRepository(targetWithIds.target)
                    .createQueryBuilder()
