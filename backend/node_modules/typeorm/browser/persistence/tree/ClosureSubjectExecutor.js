...
class ClosureSubjectExecutor {
    ...
    async insert(subject) {
        ...
        if (parent) {
            const escape = (alias) => this.queryRunner.dataSource.driver.escape(alias);
            ...
            const childEntityIds1 = subject.metadata.primaryColumns.map((column) => {
                queryParams.push(column.getEntityValue(subject.insertedValueSet ?? subject.entity));
                return this.queryRunner.dataSource.driver.createParameter("child_entity_" + column.databaseName, queryParams.length - 1);
            });
            const whereCondition = subject.metadata.closureJunctionTable.descendantColumns.map((column) => {
                ...
                const parameterName = this.queryRunner.dataSource.driver.createParameter("parent_entity_" +
    ...
    async update(subject) {
        ...
        if (parent) {
            ...
            const entityWhereCondition = subject.metadata.closureJunctionTable.ancestorColumns.map((column) => {
                ...
                const parameterName = this.queryRunner.dataSource.driver.createParameter("entity_" +
                    column.referencedColumn.databaseName, queryParams.length - 1);
