...
class CockroachDriver {
    ...
    constructor(dataSource) {
         ...
         */
        this.supportedIsolationLevels = CockroachDriver.supportedIsolationLevels;
        /**
    ...
    async afterConnect() {
        ...
        if (this.options.timeTravelQueries) {
            await this.dataSource.query(`SET default_transaction_use_follower_reads = 'on';`);
        }
        // enable experimental alter column type support (we need it to alter enum types)
        await this.dataSource.query("SET enable_experimental_alter_column_type_general = true");
    ...
    compareJsonDefaults(columnMetadata, tableColumn) {
        ...
        if (typeof jsonString === "string") {
            ...
            if (jsonString.startsWith("e'") && // CockroachDB might escape JSON/JSONB default values with e'...'
                jsonString.endsWith("'")) {
        ...
        if (typeof jsonString === "string") {
            try {
                ...
                return OrmUtils_1.OrmUtils.deepCompare(columnMetadata.default, tableColumnDefaultParsed);
            }
            catch (err) {
