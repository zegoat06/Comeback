...
class SapQueryRunner extends BaseQueryRunner_1.BaseQueryRunner {
    ...
    async query(query, parameters, useStructuredResult = false) {
        ...
        try {
            ...
            const maxQueryExecutionTime = this.driver.dataSource.options.maxQueryExecutionTime;
    ...
    async stream(query, parameters, onEnd, onError) {
        ...
        try {
            ...
            stream.on("error", (error) => {
                this.driver.dataSource.logger.logQueryError(error, query, parameters, this);
                onError?.(error);
            });
    ...
    async clearTable(tablePath, options) {
        if (options?.cascade) {
            throw new error_1.TypeORMError(`SAP HANA does not support clearing table with cascade option`);
        }
    ...
    async clearDatabase() {
        ...
        if (this.driver.options.schema) {
            ...
            const [{ currentSchema }] = await this.query(`SELECT CURRENT_SCHEMA AS "currentSchema" FROM "SYS"."DUMMY"`);
        ...
        try {
            ...
            const dropTableQueries = await this.query(selectTableDropsQuery, schemas);
    ...
    async loadTables(tableNames) {
        ...
        if (!tableNames) {
            ...
            const tablesCondition = tableNames
                ...
                .join(" OR ");
            const tablesSql = `SELECT "SCHEMA_NAME", "TABLE_NAME", "COMMENTS" FROM "SYS"."TABLES" WHERE ` +
                tablesCondition;
            dbTables.push(...(await this.query(tablesSql)));
        }
    ...
    createTableSql(table, createForeignKeys) {
        ...
        if (primaryColumns.length > 0) {
            const primaryKeyName = this.dataSource.namingStrategy.primaryKeyName(table, primaryColumns.map((column) => column.name));
            const columnNames = primaryColumns
                .map((column) => `"${column.name}"`)
                .join(", ");
    ...
    createPrimaryKeySql(table, columnNames) {
        const primaryKeyName = this.dataSource.namingStrategy.primaryKeyName(table, columnNames);
        const columnNamesString = columnNames
            .map((columnName) => `"${columnName}"`)
            .join(", ");
