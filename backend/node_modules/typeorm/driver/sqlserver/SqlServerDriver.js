...
class SqlServerDriver {
    ...
    constructor(dataSource) {
         ...
         */
        this.supportedIsolationLevels = SqlServerDriver.supportedIsolationLevels;
        /**
    ...
    createPool(options, credentials) {
        ...
        if (options.options?.isolationLevel) {
            (0, validate_isolation_level_1.validateIsolationLevel)(SqlServerDriver.supportedIsolationLevels, options.options.isolationLevel);
            isolationLevel = this.convertIsolationLevel(options.options.isolationLevel);
        }
        let connectionIsolationLevel;
        if (options.options?.connectionIsolationLevel) {
            (0, validate_isolation_level_1.validateIsolationLevel)(SqlServerDriver.supportedIsolationLevels, options.options.connectionIsolationLevel);
            connectionIsolationLevel = this.convertIsolationLevel(options.options.connectionIsolationLevel);
        }
        ...
        return new Promise((ok, fail) => {
            ...
            const poolErrorHandler = options.pool?.errorHandler ??
    ...
    convertIsolationLevel(isolation) {
        ...
        switch (isolation) {
            ...
            default:
                throw new error_1.TypeORMError(`Unknown isolation level "${isolation}"`);
        }
    }
}
