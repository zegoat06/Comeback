...
class ReturningResultsEntityUpdator {
    ...
    async update(updateResult, entities) {
        ...
        await Promise.all(entities.map(async (entity, entityIndex) => {
            ...
            if (this.queryRunner.dataSource.driver.isReturningSqlSupported("update")) {
                ...
                const result = Array.isArray(updateResult.raw)
                    ...
                    : updateResult.raw;
                const returningColumns = this.queryRunner.dataSource.driver.createGeneratedMap(metadata, result);
                if (returningColumns) {
    ...
    async insert(insertResult, entities) {
        ...
        const needToCheckGenerated = this.queryRunner.dataSource.driver.isReturningSqlSupported("insert");
        ...
        const generatedMaps = entities.map((entity, entityIndex) => {
            if (Array.isArray(insertResult.raw) &&
                this.expressionMap.extraReturningColumns.length > 0) {
                if (this.queryRunner.dataSource.driver.options.type === "oracle") {
                    insertResult.raw = insertResult.raw.reduce((newRaw, rawItem, rawItemIndex) => {
                        newRaw[this.expressionMap.extraReturningColumns[rawItemIndex].databaseName] = rawItem[0];
                        return newRaw;
                    }, {});
                }
                else if (this.queryRunner.dataSource.driver.options.type ===
                    "spanner") {
            ...
            const result = Array.isArray(insertResult.raw)
                ...
                : insertResult.raw;
            const generatedMap = this.queryRunner.dataSource.driver.createGeneratedMap(metadata, result, entityIndex, entities.length) ?? {};
